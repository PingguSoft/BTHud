...
public class LedSignBitmap {
    ...
    public void setInfo(int xres, int yres, int xres_virtual, int yres_virtual, int bpp) {
        if (mIntXRes != xres || mIntYRes != yres || mIntXResVirtual != xres_virtual ||
                mIntYResVirtual != yres_virtual || mIntBPP != bpp) {
    ...
    public void shiftBitmap2Left() {
        ...
        mBitmapBanner.getPixels(mRightPixels, 0, getXResVirtual() - 1, 1, 0, getXResVirtual() - 1,
                getYResVirtual());

        mBitmapBanner.setPixels(mRightPixels, 0, getXResVirtual() - 1, 0, 0, getXResVirtual() - 1,
                getYResVirtual());
        mBitmapBanner.setPixels(mLeftPixels, 0, 1, getXResVirtual() - 1, 0, 1, getYResVirtual());
    }
        ...
        for (int i = 0; i < mIntXResVirtual; i++) {
            for (int j = 0; j < mIntYResVirtual; j++) {
                ...
                if (Color.alpha(pixel) == 128) {
                    pixel = Color.argb(0xff, Color.red(pixel), Color.green(pixel), Color
                            .blue(pixel));
